...
delete from [Order] where convert(varchar(10),下单时间,23) in ('')
delete from [SalesSummary] where 日期 in ('')
delete from [NaturalTraffic] where [DataPeriod] in ('')
delete from [NaturalTrafficImport] where [DataPeriod] in ('')
delete from [CPCCPMSummary] where [DataPeriod] in ('') --跟[NaturalTraffic]、[SalesSummary]也有关，即这两张表任意一张有某天改动，cpccpm也要重新计算
delete from [OrderPromotion] where convert(varchar(10),下单时间,23) in ('') --跟[Order]有关


delete from [report].[ResultTimeDivisionSales] where [Date] in ('') --跟[Order]有关
delete from [report].[ResultTimeDivisionSalesProportion] where [Date] in ('')   --跟[ResultTimeDivisionSales]有关
delete from [report].ResultDailySales where [Date] in ('') --跟[SalesSummary]有关
delete from [report].[ResultNaturalTraffic] where DataPeriod in ('') --跟[NaturalTraffic]有关
delete from [report].[ResultNaturalTrafficImport] where DataPeriod in ('') --跟[NaturalTrafficImport]有关
delete from [report].[ResultCPCCPMSummary] where DataPeriod in ('') --跟[CPCCPMSummary]有关
delete from [report].[ResultDeliveryOrderPromotion] where [Date] in ('')--跟[OrderPromotion]有关
go
...
where convert(varchar(10),下单时间,23) in ('')
order by 下单时间
go
...
WHERE a.rn = 1
and convert(date,create_time-1)  in ('')
GO
...
where [CalculateType]='加总' and date  in ('')
go
...
on a.ChannelName=b.ChannelName and a.CityName=b.cityname and a.Date=b.OrderDate
where [CalculateType]='单店日均' and date  in ('')
go
...
where [CalculateType]='加总' and cityid='-1' and date  in ('')
go
...
on a.ChannelName=b.ChannelName and a.CityName=b.Market and a.Date=b.OrderDate
where [CalculateType]='单店日均' and cityid='-1' and date  in ('')
go
...
where [CalculateType]='加总' and cityid='0' and date  in ('')
go
...
on a.ChannelName=b.ChannelName and a.Date=b.OrderDate
where [CalculateType]='单店日均' and cityid='0' and date  in ('')
go
...
where [CalculateType]='加总' and [ChannelName]='total' and date  in ('')
go
...
on a.cityname=b.cityname and a.Date=b.OrderDate
where [CalculateType]='单店日均' and [ChannelName]='total' and date  in ('')
go
